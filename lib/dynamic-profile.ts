...
function inferTags(repo: { language?: string | null; name: string; description?: string | null }): string[] {
  const tags = new Set<string>(["fullstack"]);
...
function formatGeneratedLabel(timestamp: string, timezone: string): string {
...
export async function getRuntimeContent(): Promise<RuntimeContent> {
  ...
  return {
    ...
    runtime: {
      ...
      generatedAt,
      generatedLabel: formatGeneratedLabel(generatedAt, timezone),
      timezone
    }
  };
}
